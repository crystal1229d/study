...
* 함수형 컴포넌트를 최적화
* 역할 
  <strong>React.memo는 인자로 들어간 컴포넌트에 어떤 props가 입력되는지 확인하고 입력되는 모든 props의 신규 값을 확인한 뒤 이를 기존의 props의 값과 비교하도록 리액트에게 전달.
  그리고 props의 값이 바뀐 경우에만 컴포넌트를 재실행 및 재평가
  그리고 부모 컴포넌트가 변경되었지만 그 컴포넌트의 props 값이 바뀌지 않았다면 컴포넌트 실행은 건너뛴다</strong>
...
```
그리고 DemoList 에서는 오름차순으로 '정렬된' 리스트를 렌더링한다. DemoList 에 prop 으로 전달한 데이터는 정렬되지 않은 상태이다. DemoList 에 내장 정렬 메소드로 전달받은 list 를 정렬한다. 때로는 코드 또는 컴포넌트에 이렇게 성능에 민감(performance-intensive)한 로직이 있을 수 있다.
예시와 같이 리스트가 짧다면 정렬이 빠르지만 리스트가 길다면, 아니면 다른 어떤 작업에 대해 분명 시간 소요가 꽤 큰 코드가 있을 수 있다. 전체 컴포넌트가 재평가될 때마다 이러한 코드를 매번 실행하고 싶지는 않다.
불필요한 재평가 과정을 피하려면 React.memo 를 사용하면 된다고 배웠다.
React.memo 로 DemoList를 래핑하면 불필요한 동작을 막을 수 있다.
```export default React.memo(DemoList)```
이제 콘솔 로그로 DemoList RUNNING을 추가하면 초기 화면에서 출력된다. 리스트가 정렬되었다는 뜻이다. 이후에는 실행되지 말아야 한다.
아래의 버튼을 클릭하면 페이지의 title이 바뀌는데, title은 DemoList 의 일부분이므로 버튼을 클릭하면 DemoList가 재실행된다. props의 일부가 바뀌었기 때문에 당연히 재실행 되는 것이다.
React.memo 가 이 경우에는 재실행을 수행하는 것이 정상이다.
부모 컴포넌트가 재 렌더링되었으니 항상 다시 실행이 된다. 
```<DemoList title={listTitle} items={[5, 3, 1, 10, 9]} />``` DemoList 에 prop 으로 넘기는 items list 배열이 부모 컴포넌트가 실행될 때마다 지속적으로 재생성되기 때문이다. title 이 바뀌지 않더라도 상위 구성 요소가 여타 이유로 재실행되면 새로운 배열을 생성하기 때문에 DemoList 컴포넌트는 다시 실행된다.
함수와 마찬가지로, 배열 역시 객체이다. 그리고 객체는 참조값이다. 따라서 이 두 배열은 같은 원소, 같은 순서를 갖고 있어도 이 둘은 실제로는 절대 같은 배열이 아니다. 메모리 안에서 다른 위치를 점하고 있기 때문이다.
title이 바뀌기 때문에 DemoList를 리빌드해야하는 타당한 이유가 있다. 
이처럼 컴포넌트의 재실행이 재평가되는 경우가 있다. 하지만 이전에 했던 모든 작업을 반복하고 싶지 않을 때도 있을 것이다. 특히 다시 해야 할 작업이 성능에 민감한 작업일 경우 더욱 그렇다. 정렬은 컴포넌트에서 수행 가능한 대표적인 성능 집약적 작업 중 하나이다.
이에 대한 해결책은 useCallback을 사용하여 함수 객체를 저장하고 입력값이 변경될 경우에만 리빌드하는 것이다.
다른 종류의 데이터에 대해 비슷한 역할을 하는 Hook 이 useMemo Hook 이다.
useMemo는 기본적으로 저장(memoize)을 할 수 있는데 useCallback이 함수에 대한 것을 저장하듯 모든 종류의 데이터를 저장할 수 있다. (* memoize : to store (the result of a computed expression) so that it can be subsequently retrieved without repeating the computation)
```js
// DemoList.js
const { items } = props;
...
```
여기에서 useMemo를 호출하여 이 정렬의 결과를 기억하게 할 수 있다.
useMemo의 첫 번째 인자에는 함수가 들어가야 한다. 함수를 기억하게 하는 것은 아니고, 함수에서 사용자가 저장하고 싶은 것을 반환할 것이다. 예시의 경우, 정렬된 배열을 반환할 것이다.
useMemo 두 번째 인자는 의존성 배열이다. 의존성 배열을 통해 저장된 값에 변경 사항이 생길 때마다 업데이트한다. 예시의 경우, 값이 변경되고 새로운 값이 추가될 때 재정렬할 것이다. 따라서 의존성에는 item 이 들어가야 한다. item 은 props 객체에서 나오므로, destructuring (구조분해) 를 이용해 item 을 꺼낸다. 
이제 item에 변경 사항이 발생할 때만 리빌드를 진행한다.

실행하면 최초 실행후에 제목을 변경해도 여전히 계속해서 (DemoList 가) 실행된다.
왜 그런걸까? item 이 바뀌었기 때문이다.
```js
<DemoList title={listTitle} items={[5, 3, 1, 10, 9]} />
```
여기의 item은 App이 재실행될 때마다 새로운 배열을 생성하기 때문에, 재생성됩니다
기술적으로는 같은 값을 가지고, 같은 순서로 된 비슷한 배열이지만 메모리상에서는 새로운 배열이기 때문에 내포한 값이 같더라도 기존 배열과 다른 새로운 배열로 취급하는 것이다. 앞서 여러 번 언급했던 참조값(reference value)인 것이다.

```js
// DemoList.js
<DemoList title={listTitle} items={useMemo(() => [5, 3, 1, 10, 9], [])} />
```
따라서, 불필요하게 새로운 배열을 전달하는 것을 막기 위해서 useMemo를 사용한다. 배열을 반환하는 부분을 useMemo 함수로 래핑하고, 빈 의존성 배열을 추가한다. (하드 코딩된 [5,3,1,10,9] 배열을 리턴하므로 의존성이 없기 때문)
아래와 같이 JSX 코드를 깔끔하게 유지하기 위해 이 값을 외부로 꺼내온 다음 useMemo를 호출한 결과를 listItems에 저장하고 items 에 전달할 수도 있다.
```js
// DemoList.js
...
```
이렇게 함으로서 불필요한 재정렬을 막을 수 있다.
이제는 최초 실행에서 items가 정렬되고, [Change list title] 버튼을 클릭해도 useMemo 덕분에 더 이상 재정렬되지 않는것을 볼 수 있다.

useMemo는 useCallback에 비해 사용 빈도는 낮다.
함수를 기억하는 것이 훨씬 더 도움이 되고 데이터를 기억할 일보다 빈도수가 더 많기 때문이다.
물론 데이터 재계산과 같은 성능 집약적 작업 때문에 데이터를 저장해야 할 필요도 있지만 이런 경우가 아니면 별로 사용하지 않을 것이다.
useMemo를 사용해 데이터를 저장하면 이는 메모리를 사용하는 것이고 이런 함수 저장 또한 일정 성능을 사용하는 것이다. 따라서 useMemo는 여러분이 사용하는 모든 값마다 사용하는 것이 아니다.
하지만 지금과 같은 시나리오같이 뭔가를 정렬하는 경우라면 이후의 컴포넌트 업데이트에서 불필요한 정렬을 막을 수 있으므로 이를 사용하시는 것이 좋다.

어플리케이션이 느려지거나 어느 부분을 개선해야 할 때 이런 식으로 최적화가 가능한 부분을 찾아보자.